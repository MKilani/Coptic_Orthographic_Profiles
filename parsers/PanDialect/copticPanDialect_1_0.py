...
def parseCopticPanDialect(lexicalItem, verbose=False):
    ...
    ###PanDialect
    if phonemes[len(phonemes)-1] == "$" and phonemes[len(phonemes)-2] == "j|ī":
        phonemes[len(phonemes)-2] = "j|ī|ɨ"
    if phonemes[len(phonemes)-1] == "$" and phonemes[len(phonemes)-2] == "j" and isConsonant(phonemes[len(phonemes)-3]) == "cons":
        phonemes[len(phonemes)-2] = "j|ɨ"
    ###Bohairic
    # if vowelCounter > 1:
    #     if phonemes[len(phonemes)-1] == "$" and phonemes[len(phonemes)-2] == "j|ī":
